...
|---|---|---|
| Offer creation | Hormozi (Value Equation) | Suby (Godfather Offer) → Clogg (low-ticket ascension, 3-offer stack) → Brunson (Value Ladder) → Fazio (deliverable stacking) → lamxnt (marketing-first) |
| Pricing strategy | Hormozi (Value Equation) | Clogg ($27/mo → $497 → $7,200/yr ladder) → Fazio (quarterly billing, authority scale) → theroborourke → Brunson (10x value-to-price) |
| Funnel type selection | Haynes (all types) | Clogg (VSSL opt-in vs. DTA vs. low-ticket ascension) → Fazio (VSSL funnel) → brillaas → Brunson (Hook>Story>Offer) |
| Funnel build / debug | Haynes + brillaas | Suby (Secret Selling System) → Clogg (low-ticket ascension mechanics) → Fazio (VSSL buildout) → Brunson (funnel audibles) |
...
| Show rates | Haynes (80%+ system, 4 back-end systems) | Clogg (confirmed setter calls 80%+ vs. unconfirmed 20–30%) → brillaas (pre-call checklist) → Ravi (doubled show-up rate) |
...
| Sales calls | theroborourke (80/20 rule, two extremes) | Clogg (setter→closer two-step, group closing) → Fazio (two-sale model) → Haynes (group closing) → Hormozi (sales training) |
...
| Operations / scaling | Clogg (110-person team, EOS, HubSpot, 5-system stack) | Ravi (1,296 clients) → Ovens (systems thinking, flywheels) → Fazio (AI-lean ops, 5 people for 800+ clients) |
| Team comp / incentive design | Clogg (W2 + tiered commission + KPI bonus + profit share) | Ravi (scaling ops) → Hormozi (sales training) |
| LTV optimization | Clogg (LTGP stacking: Y1 44% → Y2 70% → Y3 87%) | Hormozi (Value Equation, LTV math) → Brunson (Value Ladder) |
